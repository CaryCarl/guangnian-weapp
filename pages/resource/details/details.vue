<template>
	<view class="detail-page">
		...
		<view class="detail-content" :style="{
			...
			<view class="aside-collection">
				<image v-if="collectionState" @click="onCollection()" class="img" style="width: 62rpx; height: 62rpx;"
					src="@/static/img/shoucang-ac.png" />
				<image v-else @click="onCollection()" class="img" style="width: 62rpx; height: 62rpx;"
					src="@/static/img/shoucang.png" />
				<button open-type="share" class="share-btn-wrapper">
					<image class="img mt40" style="width: 70rpx; height: 62rpx;" src="@/static/img/share.png" />
				</button>
			</view>
...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			this.getDetail();
			this.catVideoAd();
			this.getCheckMaterialFavorite(options.id)
		},
		onShareAppMessage() {
			let path = '/pages/resource/details/details?id=' + this.id
			return {
				title: "免费获取：" + this.detail.title, // 默认为小程序名称
				path, // 默认为当前页面路径
				imageUrl: this.detail.cover_image || "", // 默认为当前页面的截图
			}
		},
		methods: {
			getCheckMaterialFavorite(material_id) {
				fetch(this.$api.checkMaterialFavorite, {
					material_id
				}, 'post').then((res) => {
					if (res?.data?.code === 200 && res?.data?.collected) {
